...
def most_recent_poll_row(poll_rows, pollster, state):
    ...
    lst = []
    # Iterates through the poll_rows list setting i equal to each dict in the list
    for i in poll_rows:
	
	# sets state__ equal to the current dict's state key
	state__ = i["State"]
	# sets pollster__ equal to the current dicts pollster key
	pollster__ = i["Pollster"]
	# compares state and pollster input and if it is equal it adds the whole dict to the empty list
	if state__ == state and pollster__ == pollster:
		# adds the dictionary into the ists
		lst += [i]


   
    # checks for the empty factor
    if len(lst) == 0:
	answer = None

    # checks for the case where there is only one date (so we cannot compare two dates if there are only one
    elif len(lst) == 1:
	answer = lst[0]


    # This is the main part of function, if its not empty or just one, then i iterates through and compares dates
    else:
