...
sph = v.sphere(pos = (floor.length/float(2), 6, 0), radius = 1, color = makeRandomColors(colors))
sph.velocity = v.vector(x, y, z)


sph2 = v.sphere(pos = (floor.length/float(2) * -1, 6,0), radius = 1, color = makeRandomColors(colors))
sph2.velocity = v.vector(-x,y, z)
...
while 1:
	...
	sph2.pos = sph2.pos + sph.velocity*dt
	...
	if count == 25:
		...
		sph2.color = makeRandomColors(colors)
	...
	elif sph2.y < sph2.radius:
		sph2.velocity.y = abs(sph.velocity.y)


	else:
		sph.velocity.y = sph.velocity.y - 9.8*dt
		sph2.velocity.y = sph.velocity.y - 9.8*dt
